/**
 * 
 */
package monopoly.util.constantes;

/**
 * @author pablo
 *
 */
public class ConstantesMensaje {
	
	public static String DELIMITADOR = "&-&-&";
	public static String DELIMITADOR_OBJETOS = "&_&_&";

	public final static String CLOSE_GAME = "monopoly.message.impl.SalirJuegoMensaje";
	public final static String LOGIN = "monopoly.message.impl.LoginMensaje";
	public final static String LOGIN_RESULTADO = "monopoly.client.message.impl.LoginResultadoMensaje";
	public final static String REGISTRARME = "monopoly.message.impl.RegistrarmeMensaje";
	public final static String REGISTRARME_RESULTADO = "monopoly.client.message.impl.RegistrarmeResultadoMensaje";
	public final static String CREAR_JUEGO = "monopoly.message.impl.CrearJuegoMensaje";
	public final static String INICIAR_PARTIDA = "IniciarPartidaMensaje";

}
