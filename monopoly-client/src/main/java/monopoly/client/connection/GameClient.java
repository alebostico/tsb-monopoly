...
abstract public class GameClient {
	 ...
	 */
	public GameClient(String serverHostName, int serverPort) throws IOException {
	...
	/**
	 * This method is called when a message is received from the ServerGame.
	 * Concrete subclasses of this class must override this method to say how to
	 * respond to messages. Messages can be arbitrary Serializable objects.
	 */
	 ...
	 * connected to the ServerGame. (Note that is IS called when this client
	 * connects, so this method will be called just after the connection has
	 * been established.) The list of all connected players, including the new
	 * one, is in the protected variable connectedPlayerIDs. The method in this
	 * class does nothing.
	 * 
	 * @param newPlayerID
	...
	// ----------------------- Methods meant to be called by users of this
	// class -----------
	...
	private final ConnectionToServer connection; // Represents the network
	...
	private class ConnectionToServer {
		...
		private final int playerID; // The ID of this client, assigned by
									// the hub.
		private final Socket socket; // The socket that is connected to the
										// Hub.
		private final ObjectInputStream in; // A stream for sending messages
											// to the Hub.
		private final ObjectOutputStream out; // A stream for receiving
												// messages from the Hub.
		private final SendThread sendThread; // The thread that sends
												// messages to the Hub.
		private final ReceiveThread receiveThread; // The thread that
													// receives messages
													// from the Hub.

		private final LinkedBlockingQueue<Object> outgoingMessages; // Queue
																	// of
																	// messages
																	// waiting
																	// to be
		...
		ConnectionToServer(String host, int port) throws IOException {
			...
			extraHandshake(in, out); // Will throw an IOException if
										// handshake doesn't succeed.
